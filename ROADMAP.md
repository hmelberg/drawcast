...
## Data assets, round two: external files, code access, binaries — designed with Hans 2026-09-20, open

Three questions Hans asked the day the data-assets round landed, and the
rulings he gave. They compose into one design rather than three features, so
they belong in one round.

**1. A large asset should be a file the cast REFERS to, not one it carries.**
An asset value may be a URL that is fetched when needed. The seam already
exists and no new phase is required: `normalizeSpec` is synchronous, so a URL
cannot resolve where `"@name"` resolves today — but `render/index.ts` already
runs an async ensure phase before layout (`resolvedRenderSpec`, with
`resolvePortraits`/`resolveSources`/`resolveImages`/`resolveIcons`). A URL
asset resolves there, writes its content into the in-memory `spec.assets`,
and everything downstream is unchanged. `docs/2026-08-28-source-element-spec.md`
already worked this trade-off through for papers — IndexedDB cache, one fetch
per browser, placeholder on failure — and those conclusions transfer.

**Hans ruling: a published cast need NOT be self-contained when the asset is
large.** So a URL stays a URL in the published document; embed-on-pin becomes
an option an author may want for a small asset, never a requirement. That is a
real change to what publishing has meant until now, and it is deliberate.

A URL asset is also its own descriptor. The model sees `{url: "…/openings.csv"}`
— a handful of tokens that are MEANINGFUL, unlike the generated
`@data 50 rows — …` line — and it sidesteps both the 32 KB send threshold and
the 1 MB cap for free.

**2. A CSV asset should be readable from a `code` element's Python or R.**
Two gaps today. `CodeRunRequest` (`src/code/run.ts`) is
`{language, code, chart, paths, onStatus}` — there is no data input at all,
and the existing `{id.var}` bridge runs the other way, script -> params.
Neither `src/code/pyodide.ts` nor `src/code/webr.ts` touches a filesystem
today, though both runtimes have one (Pyodide via Emscripten's `FS`, webR via
its own).

**Hans ruling: materialize the file into the runtime filesystem when it is
read, and let Python and R open it from there.** So the script says
`pd.read_csv("openings.csv")` or `read.csv(...)` — the idiom people already
write, identical in both languages, rather than a magic injected variable.
Simplest rule: write EVERY asset in before a run rather than tracking which
the script touches; at a 1 MB ceiling that is free, and a script can then read
a file nobody declared.

This forces a correction to what shipped. `＋ Insert → Data from disk…` parses
the CSV at import and keeps only the rows, so the file itself is gone — and
re-serializing rows back to CSV is not faithful (the numeric-column rule,
quoting). **An asset should store the FILE, with rows derived when a param
references it.** Parse at the boundary, not at import. That turns out to
improve the document rather than cost it: a CSV kept as text reads better in
an `assets` fence than generated rows do, and it is byte-for-byte what the
author imported, which is the fidelity `read_csv` needs. The model-facing
descriptor can still be generated by parsing it.

**3. Arbitrary binary — parquet and friends — as base64.** Representable
already: a string value IS the existing byte path, which is what portraits and
photos have always been. Two things stop it working end to end. A params
reference to a string asset is deliberately rejected ("which is encoded bytes,
not data") — right for a photo, wrong for parquet — so an asset needs a
declared type, which the original filename probably carries well enough
(`openings.csv`, `sales.parquet`). And base64 inflates by 4/3, so the 1 MB cap
leaves roughly 750 KB of real file.

**Hans ruling: small binaries only, same size limit.** Above that the answer is
(1), a URL, not a bigger cap.

### What the three rulings produce

| | inline asset | url asset |
| --- | --- | --- |
| size | <= 1 MB (~750 KB of real binary) | unbounded |
| published | self-contained | depends on the URL staying up |
| model sees | rows under 32 KB, else a descriptor | the URL |
| code sees | the file, in the runtime FS | the file, in the runtime FS |
| params see | derived rows | derived rows |

One asset with three independent axes — where the bytes live, what they are,
who may read them. Today exactly one cell of that grid exists.

### Three things to decide while building it

- **A published cast can now break.** When a fetch fails at play time, does the
  beat draw a placeholder (the `source` element's precedent), fail loudly, or
  skip? Failing SILENTLY is the one option to argue against, on everything the
  first round taught us about silence.
- **"A string means bytes" stops being true** once a string may be CSV text or
  base64 parquet. Same type field question 3 needs.
- **The code cache key must include the asset.** A run is cached by a key over
  the code; edit the CSV and a stale result stays on screen with no sign of it.

Out of scope: authenticated or private URLs. CORS already limits this to
sources that opt in, and credentials in a published document is a different
problem.
