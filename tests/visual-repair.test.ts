...
describe("visual repair", () => {
  test("only freehand specs with a group qualify", () => {
    expect(wantsVisualRepair({ elements: [{ id: "g", type: "group", members: ["a"] }, { id: "a", type: "shape", shape: "rect", x: 81, y: 51 }], commands: [] } as never)).toBe(true);
    expect(wantsVisualRepair({ template: "supply_demand", elements: [{ id: "g", type: "group", members: ["a"] }], commands: [] } as never)).toBe(false);
    expect(wantsVisualRepair({ elements: [{ id: "a", type: "shape", shape: "rect", x: 81, y: 51 }], commands: [] } as never)).toBe(false);
  });
