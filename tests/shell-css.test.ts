...
describe("the touch tier", () => {
  it("raises every bar control to the tap minimum when there is no hover", async () => {
    ...
    for (const sel of [".pane-bar button", ".cs-bar-btn", ".sidebar-row", ".dialog-x"]) {
...
describe("the fullscreen figure", () => {
  it("keeps its own background and padding, whatever pane it lives in", async () => {
    ...
    // with `.player-figure:is(:fullscreen, .cs-faux-fs)` on specificity —
    // `:fullscreen` is a pseudo-CLASS, and `:is()` takes the weight of its
    // most specific argument — and, coming later in the file, wins. A pane
    // rule that strips `background` therefore strips it in fullscreen too,
    // leaving dark ink on the browser's black ::backdrop. `:not(:is(…))`
    // weighs the same as the `:not(:fullscreen)` it replaced, so adding the
    // faux route changed none of this arithmetic.
    ...
    for (const sel of paneRules) expect(sel).toContain(":not(:is(:fullscreen, .cs-faux-fs))");
  });
});
