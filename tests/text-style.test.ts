...
describe("effectiveTextStyle — one rule for one value", () => {
  ...
  test("the math font follows the same rule: viewer's choice, else the spec's, else Fira (Hans 2026-09-10)", () => {
    expect(effectiveTextStyle({}).mathFont).toBe("fira");
...
describe("the SVG backend draws the stamped face", () => {
  async function textNodes(d: TextDrawable): Promise<FakeNode[]> {
    ...
    try {
      ...
      const layout: LayoutResult = { drawables: [d], order: [d.id], issues: [], warnings: [], pieces: {}, pieceGroups: {}, groups: {}, attached: {}, fitGroups: {}, namedAnchors: {}, measures: {}, labelPins: {} };
