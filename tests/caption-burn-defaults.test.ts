...
describe("the upload never burns in (spec §2 ruling 4)", () => {
  test("the setting is gone from the store and the YouTube panel", async () => {
    ...
    expect(share).toContain("const embedded = await embeddedPlaylist(playlist, deps.embedDeps());");
    expect(share).toContain("const seq = exportSequence(embedded, { titleCard: deps.settings.titleCard });");
    expect(share).toContain("deps.renderVideo(seq, false, of, itemsOf(embedded).map((i) => i.spec))");
  });
  ...
  test("the download's own checkbox and setting survive", async () => {
    ...
    expect(share).toContain("const embedded = await embeddedPlaylist(doc.playlist, deps.embedDeps());");
    expect(share).toContain("const seq = exportSequence(embedded, { titleCard: videoCardCb.checked });");
    expect(share).toContain("deps.renderVideo(seq, videoBurnCb.checked, \"\", itemsOf(embedded).map((i) => i.spec))");
  });
});
