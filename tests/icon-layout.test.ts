...
describe("icon layout", () => {
  test("rings scaled to size, y flipped (svg y-down → canvas y-up), placeable with at", () => {
    ...
    const r = layoutSpec({ elements: [{ id: "a", type: "shape", shape: "rect", x: 350, y: 320, width: 100, height: 40 }, { id: "i", type: "icon", of: "box", size: 50, strokes, at: { ref: "a", side: "above", gap: 10 } }], commands: [{ draw: ["a", "i"] }] });
