 ...
 *  must not exceed `CONNECT_MAX_EDGES` (the Eridanus/Sagittarius cap), and
 *  must have no `unmatched` vertex — `ConnectKey.unmatched`'s own doc
 *  comment (render/widgets.ts) already calls that the caller's cue to
 *  refuse rather than ship a question part of whose key nobody can draw;
 *  lint agrees (lint.ts's `connect` rule), and now so does the gate.
 *  No separate `stars.length >= 2` check: `connectKey` only ever emits an
 *  edge between two matched, distinct star ids, both added to `stars` in
 *  the same walk — `edges.length >= 1` already implies at least two stars
 *  for any key this app actually produces, so a second clause here would
 *  read as a live check while never once deciding anything. */
export function connectOpens(key: { stars: readonly unknown[]; edges: readonly unknown[]; unmatched: number }): boolean {
  return key.edges.length >= 1 && key.edges.length <= CONNECT_MAX_EDGES && key.unmatched === 0;
}
