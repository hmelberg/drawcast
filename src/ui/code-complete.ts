...
//
// microdata gets no curated list at all: its verb vocabulary already lives in
// several places (prompt, docs, emulator) and a sixth copy here would be the
// one that drifts. Its scripts complete from their own words.
...
export type CompletionKind = "local" | "builtin" | "keyword";
 ...
 * pure noise. Both families comment with '#', and an odd number of quotes
 * before the caret on this line means we are inside a string.
 */
...
export function completionsFor(input: {
  ...
  const { text, caret, language, force = false, limit = 6 } = input;
  let start = caret;
  while (start > 0 && isWordChar(text[start - 1], language)) start--;
  const prefix = text.slice(start, caret);
  if (prefix === "" || /^[0-9.]/.test(prefix)) return null;
  if (!force && prefix.length < 2) return null;
  const lineStart = text.lastIndexOf("\n", start - 1) + 1;
  if (inProse(text.slice(lineStart, start))) return null;
  ...
  const afterDot = !rLike(language) && start > 0 && text[start - 1] === ".";
  const curated =
    language === "microdata" || afterDot
      ? []
      : rLike(language)
        ? [...R_BUILTINS.map((w) => ({ word: w, kind: "builtin" as const })), ...R_KEYWORDS.map((w) => ({ word: w, kind: "keyword" as const }))]
        : [...PY_BUILTINS.map((w) => ({ word: w, kind: "builtin" as const })), ...PY_KEYWORDS.map((w) => ({ word: w, kind: "keyword" as const }))];
  const locals: Completion[] = localWords(text.slice(0, start) + text.slice(caret), language).map((word) => ({ word, kind: "local" }));

  const rank: Record<CompletionKind, number> = { local: 0, builtin: 1, keyword: 2 };
