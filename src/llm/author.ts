...
// The three data engines keep their interfaces beside their data; the author
// needs those contracts as much as the ones engines.ts declares inline.
...
export const TEMPLATE_DOC_API_SCHEMA = {
  ...
  properties: {
    ...
    engines: { type: "array", items: { type: "string", enum: ["smilesdrawer", "mathjax", "chess", "geo", "anatomy", "elements", "space"] } },
    examples: {
      type: "array",
      items: {
        type: "object",
...
export function buildAuthorSystem(): Anthropic.TextBlockParam[] {
  const text = authorPromptSource
    ...
    .replaceAll(
      "{{ENGINES_SOURCE}}",
      [enginesSource, "// ---- anatomy engine types ----", anatomyTypesSource, "// ---- elements engine types ----", elementsTypesSource, "// ---- space engine types ----", spaceTypesSource].join("\n\n"),
    )
