...
- Each command sets ONE action verb (`draw` / `pause` / `wait` / `show` / `hide` / `erase` / `clear` / `highlight` / `focus` / `point` / `move` / `camera` / `animate` / `play`), optionally WITH `speak` to narrate it. `speak` alone is for the rare standalone line (e.g. the closing synthesis) — the opening line belongs ON the first draw.
