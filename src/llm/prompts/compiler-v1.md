...
- Each command sets ONE action verb (`draw` / `pause` / `wait` / `show` / `hide` / `erase` / `clear` / `highlight` / `point` / `move` / `camera` / `animate`), optionally WITH `speak` to narrate it. `speak` alone is for the rare standalone line (e.g. the closing synthesis) — the opening line belongs ON the first draw.
