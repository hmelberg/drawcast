...
- Each command sets ONE action verb (`draw` / `pause` / `wait` / `show` / `hide` / `erase` / `clear` / `highlight` / `point` / `move` / `camera`), optionally WITH `speak` to narrate it. `speak` alone is for the opening announcement and the closing synthesis.
...
- **Gestures take narration too.** `{"point": {"at": {"ref": "eq"}, "gesture": "circle"}, "speak": "This is the equilibrium."}` — the pointer circles the point while the words land. Use a narrated `point`/`highlight` for the key moments (2–4 per figure); the narrated draw carries the rest.
- **Annotate the conclusion.** An `annotation` element is a hand-drawn mark on another element: `{"id": "mark1", "type": "annotation", "target": "eq_label", "kind": "box"}` (kinds: `underline` / `box` / `circle` / `highlight` = marker sweep behind the key number / `strike` / `cross` = for rejected options; default underline for text, circle otherwise; declare it AFTER its target). Draw it at the moment of insight, narrated — `{"draw": ["mark1"], "speak": "So this is our answer."}` — or `erase` it later if it was scaffolding. To mark a gap or distance, target the arrow or stroke that IS the gap (e.g. a shortage arrow), not its text label. 1–3 annotations per figure; the moment a strike lands on a rejected option while the voice rules it out is exactly what they are for.
