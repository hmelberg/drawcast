...
- `on(event, state, scene) -> { state, effects }` — the event is
  ...
  for a finger without a steady hand.
...
periodic table, the solar system, the night sky. KNOWN_ENGINES, the full and
CLOSED set of engine names, is: smilesdrawer, mathjax, chess, geo, anatomy,
elements, space, sky.
