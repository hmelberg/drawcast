...
// commands before layout: `card` beats (spec/card.ts), then walked groups
// (spec/walk.ts). One entry point, so every consumer — render, the
...
/** Cards, then walks. The same object back when there is nothing to expand. */
export function expandSpec(spec: Spec): Spec {
  return expandWalks(expandCards(spec));
}
