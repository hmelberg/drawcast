...
// commands before layout: `card` beats (spec/card.ts), a note_sheet's
...
/** Cards, sound, then walks. The same object back when there is nothing to expand. */
export function expandSpec(spec: Spec): Spec {
  return expandWalks(expandSound(expandCards(spec)));
}
