...
`box` is a tween target: `params.box: "full"` then `{"animate": {"box": "right"}}` shrinks the fitted figure into its half over the beat (a region name stands for its rectangle in a param path — `render/params.ts`). This name-to-rectangle coercion applies only under the `box` key, never under any other string param that happens to spell one of the five names. The lint judges a code panel drawn after such an animate on the layout of the beat it is first drawn (`paramsAtFirstDraw`, `layout/layout.ts`), so "present the model large, then make room for the knobs" carries no false overlap; `pane: "controls"` is exempt from the long-script `code-use` warning, because a controls pane draws knobs, not lines. The spec validator accepts a region name (left, right, top, bottom, full) under `animate.box`, and rejects any other string or a bare number there — a number is only meaningful under `box.x`, `box.y`, `box.w` or `box.h`. And `lintCommands` warns once (`animate-box`) when a command animates `box` but the spec's `params` has no starting `box` to glide from — without one the figure jumps into the target box instead. Known limit: labels of the opening frame are still placed around the not-yet-drawn panel's ink (layout is static); invisible in practice for a panel on the empty side.
