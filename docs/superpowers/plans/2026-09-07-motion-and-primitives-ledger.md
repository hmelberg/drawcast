...
- Review-fikser: celletak 256 per pieces-element (et skjemagyldig 64 × 64-rutenett ga RangeError i lintens kvadratiske co-visibility), `n` minimum 1 (vannrette bånd via `n: 1, rows: 4`), width/height-beskrivelsene nevner pieces, en numerert barne-id (`rute_5`) ved siden av et pieces-element er valideringsfeil, `zipper`/`fan` uten sektorer varsler og legger rad, hex-doc sier at en delvis ring fylles fra første side.
