...
Addendum: fix round (5e0719c..679e6d9, pytest 401 exit 0) — the two docstrings and the else branch. PUSHED to drawcast-anvil origin/master: 8299fc8..679e6d9 (2 commits) 2026-09-06; verified origin/master == 679e6d9. Hans pulls again with "source code" (anvil.yaml changed: use_email on, allow_signup off — no db_schema change). Then: sign in with the old password (confirmed_email ticked), "Forgot password / set a password" on a link-created row, Google on an address with no row → refused with the link hint.
