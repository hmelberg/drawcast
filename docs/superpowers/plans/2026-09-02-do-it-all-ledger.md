...
- **Reversal 2026-09-03 (Hans):** "these changes should apply to both the player in the editor (preview) and the single page player." The editor-pane exclusion is gone: the override reaches every mount the app makes; exports still never pass it. Settings labels shortened to "Text size" / "Font". Hans also reported no visible effect anywhere on the live site; reproduced locally through the real UI (Settings → Largest → close → ▶ Player: 28 → 40.9) and in the `#gh=` viewer (title 55 → 80), and the live chunks carry the same code — so the report is not yet explained; diagnostic one-liner handed to Hans.
