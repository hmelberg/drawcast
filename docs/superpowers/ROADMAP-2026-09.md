...
| B14 | Voice A/B against Studio-Q — "interesting and authoritative" | R | Hans 2026-09-02, options to audition: **Chirp 3 HD** — newer family, more expressive and emotionally varied than Studio, ~a fifth of the price; try male Charon, Orus, or Iapetus (deeper, calmer) and female Kore or Aoede; downside: can drift toward a podcast/conversational feel and occasionally misplace emphasis in dense academic sentences. **Gemini-TTS** — token-priced rather than per-character, delivery steered by a natural-language prompt ("read this as an engaged university lecturer, measured pace, emphasise key terms") — the best route if the tone should shift between explaining, emphasising and summarising, and it does multi-speaker for dialogue-style lectures. *Especially Gemini might be interesting in our context.* Note for implementation: Gemini-TTS is a different API surface than the voice-name picker (a prompt + tokens, not `voice.name`), so it would be a new synthesis path beside tts.ts, not a catalog entry; Chirp voices slot straight into the existing picker/`cloudVoices` machinery today via Settings → Playback |
| B15 ✅ | Resumable narration bake — a failed publish must not lose paid audio | R | DELIVERED same day (export/bake-cache.ts): clips cached in IndexedDB the moment they exist, read before the API, key = rate+voice+speechKey, 30-day expiry, failures invisible to the bake. Original note: | Hans hit his TTS quota MID-BAKE (2026-09-02): the publish deliberately commits nothing on failure (repo stays clean), but that also discards every clip synthesized during the failed attempt — billed by Google, stored nowhere, paid again on retry. Reuse today only reads the last PUBLISHED copy. Fix: cache synthesized clips locally as they arrive (IndexedDB, like the portrait cache), keyed by speechKey+voice, and let the bake read that cache before calling the API — a retry then continues where the quota stopped it, at zero extra cost. Bounded/evictable; the cache is a wallet-protector, not a store of record |
